...
## 🌐 Деплой Frontend на GitHub Pages (бесплатно)

В репозитории есть workflow `.github/workflows/deploy-frontend-pages.yml`, который собирает `frontend` и публикует сборку на GitHub Pages.
...
1. Любой push в `main`, затрагивающий `frontend/**`, автоматически запустит workflow **Deploy Frontend to GitHub Pages**.
2. При необходимости можно запустить его вручную через **Actions → Deploy Frontend to GitHub Pages → Run workflow**.

Workflow делает следующее:
- устанавливает зависимости (`npm ci`) внутри `frontend`;
- собирает приложение (`npm run build`) с переменной `REACT_APP_API_URL=https://ai-goal-tracker-api.onrender.com`;
- загружает артефакт `frontend/build` и публикует его в среде `github-pages`.

### Шаг 3: Получить публичный URL
1. После успешного шага **Deploy to GitHub Pages** откройте вкладку **Environments → github-pages**.
2. Там появится ссылка вида `https://<ваш_аккаунт>.github.io/<ваш_репозиторий>/`.

> ⚠️ Если вы используете другой бэкенд, обновите переменную `REACT_APP_API_URL` в workflow или добавьте секрет/ENV по своему вкусу.
